    ...
    <>
      {!isInitialDelayComplete ? null : (
        <div className="initial-content-wrapper">
          ...
          {/* Filter Buttons - Floating next to dark mode toggle */}
          {hasLoadedSite && (
            <div className="floating-filter-buttons-wrapper">
              <div className="floating-filter-buttons-container">
                {getCategorizedFilters().colorblind.map((filter) => (
                  <div key={filter.id} className="floating-filter-item-wrapper">
                    <button
                      onClick={() => handleFilterChange(activeFilter === filter.id ? 'none' : filter.id)}
                      className={`floating-filter-btn ${activeFilter === filter.id ? 'active' : ''}`}
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        setSelectedFilterInfo(filter.id)
                        setFilterPopoverPosition({
                          x: rect.right + 12,
                          y: rect.top + rect.height / 2
                        })
                      }}
                      className="floating-filter-info-btn"
      ...
      <style jsx>{`
        ...
        .floating-filter-buttons-wrapper {
          position: fixed;
          top: 20px;
          right: 120px;
          z-index: 1000;
        ...
        .floating-filter-buttons-container::-webkit-scrollbar {
          height: 3px;
        }
        
        .floating-filter-buttons-container::-webkit-scrollbar-track {
          background: transparent;
        }
        
        .floating-filter-buttons-container::-webkit-scrollbar-thumb {
          background: rgba(255, 255, 255, 0.2);
          border-radius: 2px;
        }
        
        .floating-filter-item-wrapper {
          position: relative;
          display: flex;
          align-items: center;
          gap: 4px;
        ...
        .floating-filter-info-btn:hover {
